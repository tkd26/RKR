...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = RG_Conv(self.RG, conf_model['K'], conf_model['task_num'], inplanes, width, kernel_size=1, stride=1)
        self.bn1 = norm_layer(width)
        # self.conv2 = conv3x3(width, width, stride, groups, dilation)
        self.conv2 = RG_Conv(self.RG, conf_model['K'], conf_model['task_num'], width, width, kernel_size=3, stride=stride, groups=groups, dilation=dilation, padding=dilation)
        self.bn2 = norm_layer(width)
        # self.conv3 = conv1x1(width, planes * self.expansion)
        self.conv3 = RG_Conv(self.RG, conf_model['K'], conf_model['task_num'], width, planes * self.expansion, kernel_size=1, stride=1)
        ...
        if self.SFG:
            self.sfg1 = SFG_Conv(width, conf_model['task_num'])
            self.sfg2 = SFG_Conv(width, conf_model['task_num'])
            self.sfg3 = SFG_Conv(planes * self.expansion, conf_model['task_num'])
            if self.downsample is not None:
                self.sfg_down_conv = SFG_Conv(planes * self.expansion, conf_model['task_num'])
    ...
    def forward(self, x: Tensor, task: int) -> Tensor:
        ...
        out = self.conv3(out, task)
        if self.SFG: 
            out = self.sfg3(out, task=task)
        out = self.bn3(out)
