...
class RG_Conv(nn.Module):
    def __init__(self, RG, K, task_num, c_in, c_out, kernel_size, stride=3, padding=1, dilation=1, groups=1, bias=False):
        super().__init__()
...
class conv_task(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, nb_tasks=1, is_proj=1, second=0):
        ...
        self.conv = RG_Conv(True, 2, 10, in_planes, planes, stride=stride)
        self.sfg = SFG_Conv(planes, 10)
...
class ResNet(nn.Module):
    def __init__(self, block, nblocks, num_classes=[10]):
        ...
        self.fc_list = nn.ModuleList([nn.Linear(int(256*factor), class_num) for class_num in self.tasks_class])
