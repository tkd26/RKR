...
class RG_Conv(nn.Module):
    def __init__(self, RG, K, task_num, c_in, c_out, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False):
        ...
        '''
        self.LM_shape = [self.w * self.c_in, K]

        self.out_filt = self.LM_shape[0] # 出力フィルタ数
        ...
        # リストをtask0の値で初期化
        self.unc_filt_list = [nn.init.kaiming_uniform_(nn.Parameter(torch.Tensor(self.out_filt, K)))]
        self.weights_mat_list = [None]

        for task in range(task_num):
            # 非制約フィルタ（非制約フィルタ数, K）
            self.unc_filt_list.append(
                nn.init.kaiming_uniform_(
                    nn.Parameter(torch.Tensor(self.lamb_num, K))
                )
                )
            ...
            self.weights_mat_list.append(
                nn.init.kaiming_uniform_(
                    nn.Parameter(torch.Tensor((self.out_filt + task * self.lamb_num), self.lamb_rem_num))
                )
                )

        # ParameterListにしておく
        self.unc_filt_list = nn.ParameterList(self.unc_filt_list)
        self.weights_mat_list = nn.ParameterList(self.weights_mat_list)
    ...
    def forward(self, x, task: int):
        ...
        # 非制約フィルタをストアするリスト
        self.concat_unc_filter_list[task] = self.unc_filt_list[task]
        
        if task == 0:
            LM = self.unc_filt_list[0]
        else:
            # フィルタバンクの設定
            # 過去タスクの非制約フィルタをconcatする
            self.concat_unc_filter = torch.cat(self.concat_unc_filter_list[:task], dim=0)
            # フィルタバンクのリシェイプ（フィルタバンクのフィルタ数, K）→（K, フィルタバンクのフィルタ数）
            self.reshape_unc = torch.reshape(self.concat_unc_filter, (self.K, self.concat_unc_filter.shape[0]))
            # フィルタバンクと重み行列の行列積（K, piggybackフィルタ数）
            self.reshape_unc_mul_w = torch.matmul(self.reshape_unc, self.weights_mat_list[task])
            # piggybackフィルタのリシェイプ（K, piggybackフィルタ数）→（piggybackフィルタ数, K））
            self.pb_filt = torch.reshape(self.reshape_unc_mul_w, (self.reshape_unc_mul_w.shape[1], self.K))
            # 非制約フィルタとpiggybackフィルタを結合
            self.final_weight_mat = torch.cat((self.unc_filt_list[task], self.pb_filt),dim=0)
            self.final_weight_mat = self.final_weight_mat.cuda()
            LM = self.final_weight_mat

        if self.RG:
            R = torch.matmul(LM, self.RM_list[task]).view(self.w, self.h, self.c_in, self.c_out)
            R = R.permute(3, 2, 0, 1)
