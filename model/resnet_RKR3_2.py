...
class RG_Conv(nn.Module):
    def __init__(self, RG, K, task_num, c_in, c_out, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False):
        ...
        出力フィルタ数 = c_out
        非制約フィルタ数 = self.lambdas * c_out
        piggybackフィルタ数 = 出力フィルタ数 - 非制約フィルタ数
        フィルタバンクのフィルタ数 = 出力フィルタ数 + タスク番号 * 非制約フィルタ数

        非制約フィルタ(非制約フィルタ数, c_in, h, w)
        重み行列(フィルタバンクのフィルタ数, piggybackフィルタ数)
        フィルタバンク(フィルタバンクのフィルタ数, c_in, h, w)

        フィルタバンク x 重み行列(K, piggybackフィルタ数)
        '''
        if self.RG:
            self.out_filt = self.c_out # 出力フィルタ数

            self.lambdas = lambdas = 0.5 # 非制約フィルタの割合
            ...
            # 非制約フィルタのRM(h * 非制約フィルタ数, K)
            self.unc_filt_RM_list = [nn.Parameter(nn.init.kaiming_uniform_(torch.Tensor(K, self.h * self.c_out)) * self.scale)]
            self.unc_filt_RM_list += [nn.Parameter(nn.init.kaiming_uniform_(torch.Tensor(K, self.h * self.lamb_num)) * self.scale) for _ in range(task_num-1)]
            ...
            for task in range(task_num - 1):
                ...
                self.weights_mat_LM_list.append(
                    nn.init.kaiming_uniform_(
                        nn.Parameter(torch.Tensor((self.c_out + task * self.lamb_num), K))
                    )
                )
    ...
    def forward(self, x, task: int):
        if self.RG:
            ...
            if task == 0:
                # RMとLMから非制約フィルタを生成（c_out, c_in, w, h）
                self.unc_filt = torch.matmul(self.unc_filt_LM_list[task], self.unc_filt_RM_list[task]).view(self.c_out, self.c_in, self.w, self.h)
                # 非制約フィルタをストアするリストに追加
                self.concat_unc_filter_list[task] = self.unc_filt
                
                R = self.unc_filt
            else:
                # LMとRMから非制約フィルタを生成（非制約フィルタ数, 入力チャネル, h, w）
                self.unc_filt = torch.matmul(self.unc_filt_LM_list[task], self.unc_filt_RM_list[task]).view(self.lamb_num, self.c_in, self.w, self.h)
                # 非制約フィルタをストアするリストに追加
                self.concat_unc_filter_list[task] = self.unc_filt
                ...
                # フィルタバンクのリシェイプ（フィルタバンクのフィルタ数, 入力チャネル, h, w）→（（入力チャネル * h * w）, フィルタバンクのフィルタ数）
                self.reshape_unc = torch.reshape(self.concat_unc_filter, (self.c_in * self.h * self.w, self.concat_unc_filter.shape[0]))
                # フィルタバンクと重み行列の行列積（（入力チャネル * h * w）, piggybackフィルタ数）
                self.reshape_unc_mul_w = torch.matmul(self.reshape_unc, self.weights_mat)
                # piggybackフィルタのリシェイプ（（入力チャネル * h * w）, piggybackフィルタ数）→（piggybackフィルタ数, 入力チャネル, h, w））
                self.pb_filt = torch.reshape(self.reshape_unc_mul_w, (self.reshape_unc_mul_w.shape[1], self.c_in, self.h, self.w))
                # 非制約フィルタとpiggybackフィルタを結合（出力フィルタ数, 入力チャネル数, h, w）
